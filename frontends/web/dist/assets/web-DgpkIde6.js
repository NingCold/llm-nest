...
如果还有更具体的背景，欢迎补充细节，我可以给出更贴合你场景的方案。`}let I=!1;function d(){try{!I&&new URLSearchParams(location.search).get("reset")&&(I=!0,localStorage.removeItem(y),localStorage.removeItem($));const t=localStorage.getItem(y);if(t)return JSON.parse(t)}catch{}const e=L();return u(e),e}function u(e){try{localStorage.setItem(y,JSON.stringify(e))}catch{}}function L(){const e=Date.now(),t=o=>new Date(e-o).toISOString(),s=36e5,i=24*s,a={id:"s-demo-1",title:"用 Rust 写一个异步 HTTP 服务器",createdAt:t(2*s),updatedAt:t(20*6e4),messageCount:2},n=(o,p,l)=>({id:o,title:p,createdAt:t(l),updatedAt:t(l-25*6e4),messageCount:0}),r=[{id:"m-demo-u1",role:"user",content:"用 Rust 写一个简单的异步 HTTP 服务器，并解释关键点",status:"done",createdAt:e-21*6e4},{id:"m-demo-a1",role:"assistant",content:M("rust"),reasoning:v("用 Rust 写一个简单的异步 HTTP 服务器，并解释关键点").join(`
`),status:"done",thinkingMs:2840,createdAt:e-20*6e4}];return{sessions:[a,n("s-demo-2","React 组件性能优化技巧",5*s),n("s-demo-3","Tailwind CSS v4 新特性总结",1*i+3*s),n("s-demo-4","神经网络反向传播推导",3*i),n("s-demo-5","深夜食堂推荐",9*i),n("s-demo-6","项目周报模板",26*i)],messages:{"s-demo-1":r}}}function x(){try{const e=localStorage.getItem($);if(e)return JSON.parse(e)}catch{}return{currentModel:O,temperature:.7,maxTokens:4096}}function P(e){try{localStorage.setItem($,JSON.stringify(e))}catch{}}const g=new Map,h=e=>new Promise(t=>setTimeout(t,e));function b(e){const t=[];let s=0;for(;s<e.length;){const i=8+Math.floor(Math.random()*14);let a=Math.min(s+i,e.length);if(a<e.length){const n=e.indexOf(`
`,s);if(n!==-1&&n<=a)a=n+1;else{const r=e.lastIndexOf(" ",a);r>s+4&&(a=r+1)}}t.push(e.slice(s,a)),s=a}return t}const G={async init(){const e=d(),t=C,s=x(),i=[...e.sessions].sort((a,n)=>+new Date(n.updatedAt)-+new Date(a.updatedAt));return{config:s,providers:t,sessions:i,version:"0.1.0 (web demo)"}},async chat(e,t){var k;const s=e.sessionId,i=(g.get(s)??0)+1;g.set(s,i);const a=()=>g.get(s)===i,n=d(),r=n.messages[s]??[],o=e.messageId??`ai-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;t({type:"message_start",messageId:o});const p={id:`u-${Date.now()}`,role:"user",content:e.input,status:"done",...e.attachments&&e.attachments.length>0?{attachments:e.attachments}:{},createdAt:Date.now()};r.push(p);const l=_.getState().reasoningEffort!=="off",A=Date.now();let w="",S="";if(l){const c=v(e.input);for(const f of c){for(const D of b(f)){if(!a()){t({type:"cancelled",messageId:o});return}w+=D,t({type:"reasoning_delta",messageId:o,content:D}),await h(18+Math.random()*42)}await h(90+Math.random()*130)}}const R=Date.now()-A,T=((k=e.attachments)!=null&&k.length?`> 已收到 ${e.attachments.length} 个附件：${e.attachments.map(c=>c.name).join("、")}（${e.attachments.filter(c=>c.mime.startsWith("image/")).length} 张图片）。

`:"")+M(e.input);for(const c of b(T)){if(!a()){t({type:"cancelled",messageId:o});return}S+=c,t({type:"delta",messageId:o,content:c});const f=c.endsWith(`

`)?90:0;await h(10+Math.random()*30+f)}if(!a()){t({type:"cancelled",messageId:o});return}const N={id:o,role:"assistant",content:S,reasoning:l?w:void 0,status:"done",thinkingMs:l?R:void 0,createdAt:Date.now()};r.push(N);const m=n.sessions.find(c=>c.id===s);m&&((m.title==="新对话"||!m.title)&&(m.title=e.input.trim().slice(0,24)+(e.input.length>24?"…":"")),m.updatedAt=new Date().toISOString(),m.messageCount=r.length),n.messages[s]=r,u(n),t({type:"finished",messageId:o})},async cancelChat(e){g.set(e,(g.get(e)??0)+1)},async getMessages(e){return d().messages[e]??[]},async listSessions(){return[...d().sessions].sort((t,s)=>+new Date(s.updatedAt)-+new Date(t.updatedAt))},async newSession(){const e=d(),t={id:`s-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,title:"新对话",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),messageCount:0};return e.sessions.push(t),e.messages[t.id]=[],u(e),t},async deleteSession(e){const t=d();t.sessions=t.sessions.filter(s=>s.id!==e),delete t.messages[e],u(t)},async renameSession(e,t){const s=d(),i=s.sessions.find(a=>a.id===e);i&&(i.title=t,u(s))},async setConfig(e){P(e)},async setMessageFeedback(e,t,s){const i=d(),n=(i.messages[e]??[]).find(r=>r.id===t);n&&(n.feedback=s,u(i))}};export{G as webApi};
