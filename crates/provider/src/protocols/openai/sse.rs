...
impl OpenAIStream {
    ...
    fn parse_event(data: &str) -> Result<Option<LlmChunk>> {
        ...
        if let Some(content) = choice.delta.content {
            return Ok(Some(LlmChunk::Delta { content }));
        }
        ...
        loop {
                    ...
                    if let Some(data) = line.strip_prefix("data:") {
                        match Self::parse_event(data.trim()) {
                            Ok(Some(LlmChunk::Done)) => {
                                self.finished = true;
                                return Poll::Ready(Some(Ok(LlmChunk::Done)));
                            }
                            Ok(Some(chunk)) => {
            ...
            match self.inner.as_mut().poll_next(cx) {
                ...
                Poll::Ready(Some(Err(err))) => {
                    return Poll::Ready(Some(Err(ProviderError::Reqwest(err))));
                }
                Poll::Ready(None) => {
                    self.finished = true;
                    return Poll::Ready(None);
